import { formatIndonesianDate } from "./indonesian-date.ts"
...
export class XlsxWorkspaceAdapter implements WorkspaceAdapter {
  ...
  export(workspace: ProcurementWorkspace) {
    ...
    const masterRows = workspace.records.map((record, index) => ({
      ...
      PeriodeAkhir: formatIndonesianDate(record.periodEnd),
      "Tanggal PO": formatIndonesianDate(record.poDate),
      "Tanggal Memo": formatIndonesianDate(record.memoDate),
