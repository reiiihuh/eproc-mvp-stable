...
export function AddProcurementDialog({
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-4xl">
      <DialogHeader><DialogTitle className="font-display text-xl">{mode === "edit" ? "Edit Pengadaan" : "Tambah Pengadaan"}</DialogTitle><DialogDescription>{mode === "edit" ? "Perbarui data pengadaan. Nomor Request dibuat otomatis dan tidak dapat diubah." : "Nomor Request dibuat otomatis saat disimpan. Pilih requestor dan lengkapi data pengadaan."}</DialogDescription></DialogHeader>
      <div className="grid gap-7 py-2">
        <section className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Nama Requestor *"><Input list="procurement-pic-options" value={draft.picName} onChange={(event) => { const value = event.target.value; const pic = activePics.find((item) => item.name === value); if (pic) selectPic(pic); else updateDraft("picName", value) }} placeholder={activePics.length ? "Cari atau pilih Requestor (A–Z)" : "Import workbook untuk memuat Requestor"} /><datalist id="procurement-pic-options">{activePics.map((pic) => <option value={pic.name} key={pic.id}>{pic.division}</option>)}</datalist></Field>
          <Field label="Divisi"><Input value={draft.division} onChange={(event) => updateDraft("division", event.target.value)} placeholder="Terisi otomatis dari Requestor, tetap bisa dikoreksi" /></Field>
