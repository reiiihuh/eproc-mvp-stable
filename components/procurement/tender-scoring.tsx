...
export function TenderScoring({ scorecards, onChange }: { scorecards: TenderScorecard[]; onChange: (scorecards: TenderScorecard[]) => void }) {
  ...
  function validateExport() {
    if (!active?.projectName.trim()) { toast.error("Isi nama project pemilihan langsung terlebih dahulu."); return false }
    if (!weightValid) { toast.error("Total bobot teknis dan komersial harus 100%."); return false }
    if (completeVendors.length < 2) { toast.error("Isi minimal dua vendor beserta penawaran akhirnya."); return false }
    return true
  }
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5">
    ...
    <Card className="border-slate-200 shadow-sm">
      ...
      <CardContent className="grid gap-4 pt-5 md:grid-cols-2 xl:grid-cols-4">
        <Field label="Nama project pemilihan langsung *" className="md:col-span-2"><Input value={active.projectName} onChange={(event) => updateScorecard({ projectName: event.target.value })} placeholder="Contoh: Annual Penetration Testing 2026" /></Field>
    ...
    <Card className="border-slate-200 shadow-sm">
      ...
      <CardContent className="space-y-3 pt-5">
        {active.vendors.map((vendor, index) => <div key={vendor.id} className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50/50 p-4 xl:grid-cols-[50px_1.1fr_1fr_1fr_.75fr_1.5fr_auto]">
          ...
          <Field label="Penawaran akhir (excl. PPN)"><Input type="number" min="0" value={vendor.finalPrice || ""} onChange={(event) => updateVendor(vendor.id, { finalPrice: Number(event.target.value) })} /></Field>
